...
class JualSampahPage extends StatefulWidget {
  // Widget stateful untuk halaman jual sampah
  final List<SampahItemModel>?
  cartItems; // Properti untuk menyimpan item keranjang
  const JualSampahPage({
    super.key,
    this.cartItems,
  }); // Konstruktor dengan parameter opsional cartItems
...
class _JualSampahPageState extends State<JualSampahPage> {
  // State dari JualSampahPage
  // Controllers untuk TextField
  final TextEditingController addressController =
      TextEditingController(); // Controller alamat penjemputan
  ...
  void initState() {
    ...
    _currentCartItems = List.from(
      widget.cartItems ?? [],
    ); // Salin item keranjang dari widget
    _calculateTotalPoint(); // Hitung total harga awal
  }

  void _calculateTotalPoint() {
    // Fungsi untuk menghitung total point keranjang
  ...
  Future<void> _selectDate(BuildContext context) async {
    // Fungsi untuk memilih tanggal penjemputan
  ...
  Future<void> _submitTransaction() async {
    ...
    List<Map<String, dynamic>> wastesData =
        _currentCartItems.map((item) {
          return {'waste_id': item.id, 'quantity': item.quantity};
        }).toList();
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        debugPrint(
          'Error API Response (Status: ${response.statusCode}): ${response.body}',
        );
        ScaffoldMessenger.of(context).showSnackBar(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            final updatedCart =
                                await Navigator.push<List<SampahItemModel>>(
                                  // Navigasi ke DaftarSampahPage
                                  context,
                                  MaterialPageRoute(
                                    builder:
                                        (context) => DaftarSampahPage(
                                          initialCartItems: _currentCartItems,
                                        ), // Kirim item keranjang
                                  ),
                                );
                            if (updatedCart != null) {
                              // Jika ada perubahan keranjang
                              setState(() {
                                _currentCartItems = List.from(
                                  updatedCart,
                                ); // Update keranjang
            ...
            _buildSectionHeader(
              "Alamat Penjemputan",
            ), // Header alamat penjemputan
            const SizedBox(height: 8), // Spasi
            Container(
              // Container input alamat
            ...
            _buildSectionHeader("Detail Sampah"), // Header detail sampah
            const SizedBox(height: 8), // Spasi
            Container(
              padding: const EdgeInsets.all(16), // Padding
              decoration: BoxDecoration(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(
                    _totalPointKeranjang.toStringAsFixed(
                      0,
                    ), // Tampilkan total point
                    style: TextStyle(
                      fontSize: 18,
                      fontWeight: FontWeight.bold,
              ...
              child: Image.network(
                ...
                errorBuilder:
                    (context, error, stackTrace) => Container(
                      width: 70,
                      height: 70, // Ukuran container error
